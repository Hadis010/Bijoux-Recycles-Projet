...
if ($resultat == false) { // Erreur lors de l’exécution de la requête d'insertion du compte
    ...
    exit();
} else if($row1['pfl_role'] == 'G') {

    
      echo '<table class ="table">';
      echo "<tr><th>Pseudo</th><th>Nom</th><th>Prénom</th><th>Rôle</th><th>Validité</th><th>ActionsA</th><th>ActionsG</th></tr>";

      while ($row = $resultat->fetch_assoc()) {
      echo "<tr>";
      echo "<td>" . htmlspecialchars($row['cpt_pseudo']) . "</td>";
      echo "<td>" . htmlspecialchars($row['pfl_nom']) . "</td>";
      echo "<td>" . htmlspecialchars($row['pfl_prenom']) . "</td>";
      echo "<td>" . htmlspecialchars($row['pfl_role']) . "</td>";
      echo "<td>" . htmlspecialchars($row['pfl_validite']) . "</td>";
      echo "<td><form method='post' action='comptes_action.php'>";
      echo "<input type='hidden' name='pseudo' value='" . htmlspecialchars($row['cpt_pseudo']) . "'/>";
      $buttonText = $row['pfl_validite'] == 'A' ? 'Désactiver' : 'Activer';
      echo "<input type='submit' name='b_validite' value='$buttonText'/>";
      echo "</form></td>";
      echo "<td><form method='post' action='comptes_action.php'>";
      echo "<input type='hidden' name='pseudo' value='" . htmlspecialchars($row['cpt_pseudo']) . "'/>";
      $buttonText1 = $row['pfl_role'] == 'G' ? 'Membre' : 'Gestionnaire';
      echo "<input type='submit' name='b_role'  value='$buttonText1'/>";
      echo "</form></td>";
      echo "</tr>";
    }
