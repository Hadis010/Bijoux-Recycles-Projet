...
<?php
    ...
    if(isset($_POST['delete']) ) {
        ...
        $sql = " SELECT suj_intitule,fic_label,hyp_numero FROM t_fichier_fic RIGHT JOIN t_sujet_suj USING (suj_numero) LEFT JOIN t_association_asso USING (fic_numero) WHERE suj_intitule = '" . $sn. "';";
        ...
        if ($resultat == false) { // Erreur lors de l’exécution de la requête d'insertion du compte
            // La requête a échoué
            ...
            if($resultat->num_rows >0){
              ...
              $si = $ligne['suj_intitule'];
              $fi = $ligne['fic_label'];
              $hi = $ligne['hyp_numero'];
              while($ligne = $resultat->fetch_assoc()){
                if(isset($_POST['s_intitule'])) {
                    // Supprimer la fiche sélectionnée
                            if($hi){
                                $update_sql1 = "DELETE FROM t_association_asso WHERE hyp_numero = '".$hi."' AND fic_numero IN (SELECT fic_numero FROM t_fichier_fic WHERE fic_label = '". $fi ."');";
                                $delete_result1 = $mysqli->query($update_sql1);
                                $update_sql2 = "DELETE FROM t_hyperlien_hyp WHERE hyp_numero = '".$hi."'"; 
                                $update_result2 = $mysqli->query($update_sql2);
                                if($delete_result1 === false && $update_result2 === false ) {
                                    echo "Erreur: Impossible de supprimer ce sujet1 \n";
                                    exit();
                                }
                                }
                                if($fi){
                                    $update_sql3 = "DELETE FROM t_fichier_fic WHERE fic_label = '".$fi."'"; 
                                    $update_result3 = $mysqli->query($update_sql3);
                                    if($update_result3 === false) {
                                        echo "Erreur: Impossible de supprimer ce sujet2 \n";
                                        exit();
                                    }}
                                if(!($fi) && !($hi) && $si){
                                $update_sql4 = "DELETE FROM t_sujet_suj WHERE suj_intitule = '".$si."'"; 
                                $update_result4 = $mysqli->query($update_sql4);
                                if($update_result4 === false) {
                                echo "Erreur: Impossible de supprimer ce sujet3 \n";
                                exit();
                              }
                            } 
                          }
                        } 
                                header("Location: admin_sujets.php"); // Redirection vers la page d'admin
                                exit();
                            
                
                //else{
                // aucune ligne retournée et donc le compte n'existe
                //echo "sujet inconnu !";
        ...
        // Vérification si le formulaire est soumis
    else if(isset($_POST['submit']) && $_POST['titre']) {
            ...
            // Requête SQL pour ajouter le sujet
            $sql15 = "INSERT INTO t_sujet_suj (suj_numero,suj_intitule,suj_date,cpt_pseudo) VALUES (NULL,'".$titre."', CURRENT_DATE(),'".$pseudo."');";
            echo($sql15);
            // Exécution de la requête d'ajout d'un compte dans la table des comptes
            $resultat15 = $mysqli->query($sql15);
            if ($resultat15 == false) { // Erreur lors de l’exécution de la requête d'insertion du compte
                // La requête a échoué
                echo "Error: La requête a échoué \n";
                echo "Query: " . $sql . "\n";
                echo "Errno: " . $mysqli->errno . "\n";
            ...
            if ($mysqli->affected_rows == 1) {
                ...
                // Affichage d'un message d'erreur en cas d'échec de l'ajout
                echo "Erreur: Impossible d'ajouter le sujet \n";
                exit();
            }
        }
            ...
            // Si le formulaire n'est pas soumis et aucune déconnexion demandée,
            // afficher un message d'erreur
            echo "Veuillez saisir le titre du sujet !";
            echo "<br /><a href=\"./admin_sujets.php\">Cliquez ici pour réafficher le formulaire</a>";
            exit();
        }
    ?>
...
<script type="text/javascript" id="www-widgetapi-script" src="../js/www-widgetapi.js" async=""></script><script async="" src="../inscription/iframe_api"></script></head>
<style>
.cid-u6fkJMRQpR  {
  z-index: 1000;
  width: 100%;
    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse opacityScroll" id="navbarSupportedContent">
        <ul class="navbar-nav nav-dropdown" data-app-modern-menu="true">
          <li class="nav-item">
            <a class="nav-link link text-black display-4" href="./../index.php">Accueil</a>
          </li>
          <li class="nav-item">
            <a class="nav-link link text-black display-4" href="./recapitulatif.php" aria-expanded="false">Recapitulatif</a>
          </li>
          <li class="nav-item">
