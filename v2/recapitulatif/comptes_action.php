...
<?php
    ...
    if(isset($_POST['b_validite']) || isset($_POST['b_role'])) {
            $id = $_POST['pseudo'];
        ...
        if ($resultat == false) { // Erreur lors de l’exécution de la requête d'insertion du compte
            ...
            if($resultat->num_rows == 1){
                if(isset($_POST['b_validite'])) {
                    // Activer ou désactiver le compte en fonction de sa validité actuelle
                        $validite = ($ligne["pfl_validite"] == 'A') ? 'D' : 'A';
                        $update_sql1 = "UPDATE t_profil_pfl SET pfl_validite = '" . $validite . "' WHERE cpt_pseudo = '" . $id . "'"; 
                        $update_result1 = $mysqli->query($update_sql1);
                        if($update_result1 === false) {
                            echo "Erreur: Impossible de mettre à jour  du profil \n";
                            exit();
                        } else {
                            header("Location: admin_accueil.php"); // Redirection vers la page d'admin
                            exit();
                        }
                } elseif(isset($_POST['b_role'])) {
                    $role_u =  ($ligne["pfl_role"] == 'G') ? 'M' : 'G';
                    $update_sql2 = "UPDATE t_profil_pfl SET  pfl_role = '".$role_u."' WHERE cpt_pseudo = '" . $id . "'"; 
        ...
        echo "Veuillez saisir le pseudo de l'utilisateur !";
        echo "<br /><a href=\"./admin_accueil.php\">Cliquez ici pour réafficher
        le formulaire</a>";
        exit();
    }
...
<?php
    ...
    echo "<br>";
    echo "Prénom : ". $_SESSION['login'];
    echo "<br>";
    echo "Nom : ". $_SESSION['login'];
    echo "<br>";
    echo "ton role : vous êtes administrateur";?>
