...
/* Vérification ci-dessous à faire sur toutes les pages dont l'accès est
    ...
    if(!isset($_SESSION['role']))  //A COMPLETER pour tester aussi le rôle...
    {
...
//on vérifie si les champs ont été bien remplis
        if($_POST["pseudo"]&&$_POST["mdp"]){
            ...
            $sql = "SELECT cpt_pseudo,cpt_mot_de_passe,pfl_role,pfl_validite FROM t_compte_cpt JOIN t_profil_pfl USING (cpt_pseudo) WHERE
            cpt_pseudo='" . $id . "' AND cpt_mot_de_passe=MD5('" . $mdp . "');";
            ...
            if ($resultat == false) { // Erreur lors de l’exécution de la requête d'insertion du compte
                ...
                if($resultat->num_rows == 1){
                    ...
                    else if ($ligne["pfl_validite"]=='D' ){
                      // aucune ligne retournée
                      // => le compte n'est pas valide
                      echo "Cette partie est resérvée aux gestionnaires !";
                      echo "<br /><a href=\"./session.php\">Merci de revenir vers
                      le formulaire</a>";
                      exit();
                    } else{
                      // aucune ligne retournée
