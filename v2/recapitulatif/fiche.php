    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse opacityScroll" id="navbarSupportedContent">
        <ul class="navbar-nav nav-dropdown" data-app-modern-menu="true">
          ...
          <li class="nav-item">
            <a class="nav-link link text-black display-4" href="./session.php">Admin</a>
          </li>
        </ul>
...
if(isset($_GET['code'])){
    ...
    // Vérification de la longueur de la chaîne récupérée
    if(strlen($code_fiche) != 12) {
        echo '<div class="alert alert-danger" role="alert">';
        echo "Erreur: Le paramètre 'code' doit contenir exactement 12 caractères.";
        echo '</div>';
        exit();
    }
} else {
    echo '<div class="alert alert-danger" role="alert">';
    echo "Erreur: Le paramètre 'code' est absent dans l'URL.";
    echo '</div>';
    exit();
}

$requete = "SELECT fic_numero,fic_label, fic_contenu, fic_ficimg, fic_code, fic_etat,suj_intitule FROM t_fichier_fic JOIN t_sujet_suj USING (suj_numero) WHERE fic_code = '".$code_fiche."';";
$result = $mysqli->query($requete);
$suj = $result->fetch_assoc();
...
if ($result == false) {
    ...
    echo '<h1>' . $suj['suj_intitule'] . '</h1>';
    echo '<h2>' . $suj['fic_label'] . '</h2>';
    echo '<img src="../images/'. $suj['fic_ficimg'] . '" alt="Miniature" class="im1g mx-auto d-block" style="max-width: 250px; height: auto;">';
    echo '<h5>' . $suj['fic_contenu'] . '</h5>';
    echo '</div>';  

    $request = "SELECT hyp_url,hyp_numero FROM t_fichier_fic JOIN t_association_asso USING (fic_numero) 
    JOIN t_hyperlien_hyp USING (hyp_numero) WHERE fic_code = '".$code_fiche."';";
    $result_bis = $mysqli->query($request);
    ...
    if ($result_bis == false) {
        ...
        if ($result_bis->num_rows > 0) {
            // Récupère les résultats dans $hyp
            echo '<div style="margin: 20px auto; width: 80%; border: 1px solid #ccc; padding: 10px;">';
            echo '<h3>Liens associés :</h3>';
            while ($hyp = $result_bis->fetch_assoc()){
            echo("<a href='".$hyp['hyp_url']."'> '". htmlspecialchars($hyp['hyp_url']) ."' </a>");
          echo "<br>";}
        } else {
